% http://www.texfaq.org/FAQ-compjobnam
\def\StripPrefix#1>{}
\def\jobis#1{FF\fi
  \def\predicate{#1}%
  \edef\predicate{\expandafter\StripPrefix\meaning\predicate}%
  \edef\job{\jobname}%
  \ifx\job\predicate
}

\newif\ifshowthumb
\if\jobis{Cookbook-Digital}
	\documentclass[openany]{book}
	\usepackage[margin=.5in]{geometry}
	\showthumbfalse
\else
	\documentclass[twoside, openany]{book}
	\usepackage[top=1in, bottom=1in, inner=1in, outer=.5in]{geometry}
	\showthumbtrue
\fi

\if\jobis{Cookbook-Tested}
	\newcommand{\untested}[1]{}
\else
	\newcommand{\untested}[1]{#1}
\fi


\usepackage[T1]{fontenc}

\usepackage[contents, nonumber]{cuisine}
\usepackage{hyperref}
\hypersetup{colorlinks, citecolor=blue, filecolor=black, linkcolor=black, urlcolor=black}

\usepackage{subfiles}
\usepackage{thumbs}
\usepackage{nicefrac}
\let\fr\nicefrac
\usepackage{siunitx}
\usepackage{etoolbox}

% Local sty files
\usepackage{formatting}
\usepackage[annatar]{usetengwar}
\usepackage{luamacros}


\newcommand{\dish}[1]{%
\begin{minipage}{\pagewidth}%
\setcounter{footnote}{0}%
\setcounter{mpfootnote}{0}%
\subfile{entries/#1}%
\end{minipage}%
\vfill%
}

\newcommand{\ap}{all-purpose}
\newcommand{\ww}{whole wheat}
\newcommand{\wwp}{whole wheat pastry}
\newcommand{\wwb}{whole wheat bread}
\newcommand{\tooth}[1]{until a toothpick comes out clean, about #1 minutes.}

% Referencing a page will basically always be with "page X"
\newcommand{\reciperef}[1]{page~\pageref{#1}}

\newcommand{\thumbmark}[1]{%
\ifshowthumb%
\addthumb{#1}{\Roman{chapter}}{black}{gray}%
\fi%
}
\newcommand{\preheat}[2][sentence]{\ifstrequal{#1}{sentence}%
	{Preheat oven to \tempf{#2}.}%
{\ifstrequal{#1}{beginning}%
	{Preheat oven to \tempf{#2}}%
{\ifstrequal{#1}{fragment}%
	{preheat oven to \tempf{#2}}%
{}}}}



\title{Family Cookbook}
\author{Thurmes Family}


% TODO: Correct times. At best the current times are approximate.

\begin{document}

\frontsetup

{
	\bfseries \tableofcontents
}

\mainsetup

\chapter{Breakfast}
\thumbmark{Breakfast}


\section{Egg}

\dish{FarmersEggs}


\section{Not Egg}

\dish{DadsGranola}

\dish{SausageGravy}

\untested{\dish{SavoryOatmeal}}

\dish{SpicedPancakes}





\chapter{Dinner}
\thumbmark{Dinner}


\section{Full Meals}

\dish{AlmondChickenAndRice}

\dish{BabyBackRibs}

\untested{\dish{BakedMacAndCheese}}

\dish{BakedRavioli}

\dish{BeefRoast}

\dish{BeefStew}

\untested{\dish{BiscuitsAndGravy}}

\dish{BLTs}

\dish{BroccoliCheeseSoup}

\untested{\dish{Bulgogi}}

\untested{\dish{CaramelizedOnionMushroomAndBeefCalzones}}

\dish{ChickenAndDumplingSoup}

\untested{\dish{ChickenBreastsWithSteamRoastedParsnips}}

\untested{\dish{ChickenWithOreganoAndMushrooms}}

\dish{ChuckwagonCasserole}

\dish{EnhancedGrilledCheese}

\dish{FriedRice}

\dish{Goulash}

\dish{GreenTortellini}

\dish{HamburgerPasties}

\dish{Hamburgers}

\untested{\dish{JasmineRiceEdamameAppleBowls}}

\untested{\dish{LentilBolognese}}

\dish{MapleRoastedPorkTenderloin}

\dish{MongolianBeef}

\untested{\dish{NuttyPortobelloChili}}

\untested{\dish{OnePanBakedChicken}}

\untested{\dish{OvenFriedChicken}}

\untested{\dish{PadThai}}

\dish{ParmesanChicken}

\dish{RosemaryChicken}

\untested{\dish{SageBreadedChicken}}

\dish{SalamiCheeseStromboli}

\dish{ScallopedPotatoes}

\untested{\dish{SesameGingerPorkAndQuinoa}}

\untested{\dish{SlowCookerChanaMasala}}

\dish{ShepherdsPie}

\untested{\dish{SloppyJoes}}

\dish{Spaghetti}

\untested{\dish{SpicyJambalaya}}

\dish{SquashSoup}

\untested{\dish{SrirachaChicken}}

\dish{StirFry}

\dish{ThanksgivingDinner}

\untested{\dish{TurkeySageMeatballs}}

\dish{VegetableBeefSoup}

\untested{\dish{VegetarianBeanChili}}

\untested{\dish{VegetarianChili}}

\dish{WaxBeanSoup}

\untested{\dish{WashtunkalaStew}}


\section{Components}

\dish{BasicPastaDough}

\dish{BasilPesto}

\dish{BoiledEggs}

\dish{Dumplings}

\untested{\dish{LimeGingerVinaigrette}}

\untested{\dish{PeanutSauce}}

\dish{PizzaSauce}

\dish{Quinoa}

\dish{Rice}

\dish{SpaghettiSauce}

\dish{SquashCustard}

\dish{ThanksgivingStuffing}

\dish{WholeWheatPizzaDough}






\chapter{Sides}
\thumbmark{Sides}


\section{Breads}

\dish{Biscuits}

\dish{Bruschetta}

\untested{\dish{CheddarChiveCornbread}}

\dish{Cornbread}

\dish{DinnerRolls}

\untested{\dish{HarvestBread}}

\untested{\dish{HoneyRolls}}

\untested{\dish{HoneyWheatRolls}}

\dish{OatBuns}

\untested{\dish{SouthernButtermilkBiscuits}}

\untested{\dish{SweetSquashCornMuffins}}

\untested{\dish{WhiteBread}}


\section{Fruits and Vegetables}

\dish{BakedPotatoes}

\dish{BrusselsSprouts}

\dish{Corn}

\dish{CornAndBeanSalad}

\dish{CranberrySauce}

\dish{FruitSalad}

\dish{GarlicPotatoes}

\dish{GreenBeans}

\dish{GreenBeansWithCream}

\dish{Latkes}

\dish{MashedPotatoes}

\dish{MashedPotatoesInstantPot}

\untested{\dish{RoastPotatoesMetric}}

\dish{RoastVegetables}






\chapter{Snacks}
\thumbmark{Snacks}

\section{Drinks}

\untested{\dish{GreenTeaSmoothie}}

\dish{HotCocoaMix}

\dish{MixedBerrySmoothie}


\section{Dips}

\dish{BlackBeanDip}

\untested{\dish{NutDip}}

\untested{\dish{PumpkinButter}}

\dish{SerranoSalsa}


\section{Morsels}

\dish{Popcorn}





\chapter{Desserts}
\thumbmark{Desserts}


\section{Cookies}

\dish{BethsButterSugarCookies}

\dish{BrownButterChocolateChipCookies}

\untested{\dish{ChocolatePeanutTartlets}}

\dish{CoconutChocolateChocolateChipCookies}

\dish{Gingerbread}

\dish{Lebkuchen}

\dish{PrideOfIowa}

\dish{PumpkinCookies}

\dish{RussianTeaCakes}

\untested\dish{ScotchShortbread}

\untested\dish{Snickerdoodles}


\section{Quick Breads}

\dish{BananaBread}

\dish{BananaChocolateMuffins}

\untested{\dish{BlueberryMuffins}}

\untested{\dish{BlueberryWalnutMuffins}}

\untested{\dish{BranMuffins}}

\untested{\dish{Brownies}}

\untested{\dish{ChristmasLoaf}}

\untested{\dish{FudgyBrownies}}

\untested{\dish{GingerBiscuits}}

\dish{Lembas}

\untested{\dish{OatmealWalnutBread}}

\dish{OrangeBread}

\untested{\dish{NutellaBrownies}}

\untested{\dish{PotatoAppleBread}}

\dish{PumpkinBread}

\untested{\dish{PumpkinWalnutMuffins}}

\untested{\dish{RaspberryScones}}

\untested{\dish{RedVelvetBrownies}}

\dish{RhubarbCoffeeCake}

\dish{Scones}

\untested{\dish{SoulCakes}}

\untested{\dish{ZucchiniBread}}

\untested{\dish{ZucchiniChocolateMuffins}}


\section{Pies}

\dish{PieDough}

\dish{DeepDishApplePie}

\dish{DeepDishCherryPie}

\untested{\dish{FreshBlueberryTart}}

\dish{PeachPie}

\dish{PumpkinPie}

\dish{DeepDishRhubarbPie}



\section{Fruit Desserts}

\dish{Apfelkuchen}

\untested{\dish{AppleDumplings}}

\dish{BakedApples}

\untested{\dish{BavarianAppleCake}}

\dish{BerryCrumble}

\untested{\dish{BerryJam}}

\untested{\dish{BoiledApples}}

\untested{\dish{CherryRaspberrySorbet}}

\dish{CoconutRicePudding}

\untested{\dish{LemonBars}}


\section{Sides and Toppings}

\untested{\dish{Baklava}}

\untested{\dish{ChocolateFudge}}

\untested{\dish{Macaroons}}

\untested{\dish{PeanutBrittle}}

\dish{RhubarbSauce}

\dish{WhippedCream}


\section{Cake}

\untested{\dish{BaseDessertBread}}

\dish{ChocolateCake}

\dish{ChocolateButtercreamFrosting}

\untested{\dish{CinnamonRolls}}

\untested{\dish{CinnamonStar}}

\untested{\dish{CranberryNutCake}}

\untested{\dish{DarkChocolateCheesecake}}

\untested{\dish{LemonPolentaCake}}

\untested{\dish{RaspberryCheesecake}}


\chapter{Appendix}
\thumbmark{Appendix}


\section{Measurement Conversions}

\input{volume-table.tex}


\begin{table}[h]
	\centering
	\label{tab:extraunits}
	\caption{Extra units in use in recipes}
	\begin{tabular}{cc}
		quart & 2 pints \\
		gallon & 4 quarts \\
		packet yeast & \fr52 teaspoons
	\end{tabular}
\end{table}

\begin{table}[h]
	\centering
	\label{tab:temp}
	\caption{Temperature}
	\begin{tabular}{cc}
		\si{\degree}F & \si{\degree}C \\ \midrule
		250           & 130           \\
		300           & 150           \\
		325           & 170           \\
		350           & 180           \\
		375           & 190           \\
		400           & 200           \\
		425           & 220
	\end{tabular}
\end{table}


\begin{table}[ht]15,000
	\centering
	\label{tab:mass}
	\caption{Mass and Weight}
	\begin{tabular}{cc}
		g    & oz             \\ \midrule
		10   & \fr14          \\
		15   & \fr12          \\
		25   & 1              \\
		50   & \fr74          \\
		75   & \fr{11}{4}     \\
		100  & \fr72          \\
		150  & \fr{11}{2}     \\
		200  & 7              \\
		250  & 9              \\
		300  & \fr{21}{2}     \\
		350  & 12             \\
		400  & 14             \\
		450  & 16 (1 lb)      \\
		500  & 18             \\
		700  & 24             \\
		1000 & 36 (2 lb 4 oz)
	\end{tabular}
\end{table}

\input{density-table.tex}


\end{document}
